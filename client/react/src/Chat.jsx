...
function Chat() {
  ...
  const [chattingWithStatus, setChattingWithStatus] = useState(false);
  ...
  useEffect(() => {
    const fetchChattingWithStatus = async () => {
      try {
        if (chattingWith != "") {
          const chattingWithUID = await getUserIDFromUsername(chattingWith);
          const status = await getOnlineStatus(chattingWithUID);
          console.log(status);
          setChattingWithStatus(status);
        }
      } catch (error) {
        console.log(error.message);
      }
    };
    fetchChattingWithStatus();
  }, [chattingWith]);
                      ...
                      {chattingWith && (
                        <>
                          <Box
                            ...
                            h={"14px"}
                            borderRadius={"7px"}
                            bgColor={chattingWithStatus ? "#14d941" : "#c5c7c9"}
                            marginTop={"5px"}
                            marginRight={"10px"}
                          ></Box>
                          <Text color={"#8a8a8a"}>
                            {chattingWithStatus ? "Online" : "Offline"}
                          </Text>
                        </>
                      )}
                      <Spacer></Spacer>
                    ...
                    >
                      {chattingWith ? (
                        allMessages.map((message, index) => (
                          <Box
                            ...
                            style={{
                              ...
                              background:
                                message.senderUsername === username
                                  ? "#47A9F9"
                                  : "#F3F3F3", // Set the background color
                              color:
                                message.senderUsername === username
                                  ? "white"
                          ...
                          >
                            <Text maxW="100%" whiteSpace="pre-wrap">
                              {message.senderUsername === username
                                ? "You: "
                                : ""}
                              &nbsp;{message.text}
